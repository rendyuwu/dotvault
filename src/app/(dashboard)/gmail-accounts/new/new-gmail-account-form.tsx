...
import {
  ...
  type GmailAccountActionState,
} from "@/lib/gmail/accounts-actions";
import { maxAliasCount, normalizeGmail } from "@/lib/gmail/normalize";

type Props = {
  existingCanonicalEmails: string[];
};

const initialState: GmailAccountActionState = {};

export function NewGmailAccountForm({ existingCanonicalEmails }: Props) {
  ...
  const duplicate = validation?.ok
    ? existingCanonicalEmails.includes(validation.canonicalEmail)
    : false;
  const isValid = validation?.ok && !duplicate;

  function handleSubmit(e: { preventDefault(): void }) {
    setSubmitted(true);
    if (!isValid || !label.trim()) e.preventDefault();
  }
