...
import {
  ...
  type GenerateAccountOption,
  type GeneratePreviewStats,
} from "@/lib/gmail/generate-actions";
import { maxAliasCount } from "@/lib/gmail/normalize";
import type { GeneratedAlias } from "@/lib/gmail/generate";

type GenerateClientProps = {
  accounts: GenerateAccountOption[];
};

export function GenerateClient({ accounts: initialAccounts }: GenerateClientProps) {
  ...
  const [previewStats, setPreviewStats] = useState<GeneratePreviewStats | null>(
    null
  );
  ...
  const totalPossible = selectedAccount
    ? (previewStats?.totalPossible ?? maxAliasCount(selectedAccount.localPart.length))
    : 0;
  const alreadySaved = selectedAccount
    ? (previewStats?.alreadySaved ?? selectedAccount.aliasCount)
    : 0;
  const remaining = selectedAccount
    ? (previewStats?.remaining ?? Math.max(totalPossible - alreadySaved, 0))
    : 0;
  ...
  function handleSave() {
    ...
    setError(null);
    startTransition(async () => {
      const result = await saveGeneratedAliasesAction({
        gmailAccountId: selectedAccount.id,
        aliases: preview,
      });
      ...
      setAccounts((current) =>
        current.map((account) =>
          account.id === selectedAccount.id
            ? { ...account, aliasCount: account.aliasCount + result.saved }
            : account
        )
      );
      router.refresh();
      ...
      <Card>
        <div className="space-y-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setCount(selectedAccount ? Math.min(nextCount, Math.max(maxCount, 1)) : nextCount);
                }}
          ...
          <Button
            ...
            disabled={!selectedAccount || count < 1 || remaining <= 0 || isPending}
            className="gap-1.5"
          >
            <Sparkles size={14} />
            Generate Preview
      ...
      {previewStats?.shortage && (
        <div className="flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/5 px-4 py-3">
          ...
          <p className="text-sm text-amber-400">
            Only {previewStats.remaining.toLocaleString()} aliases available. Showing{" "}
            {previewStats.generated.toLocaleString()} instead of{" "}
            {previewStats.requested.toLocaleString()} requested.
          </p>
        </div>
      )}
